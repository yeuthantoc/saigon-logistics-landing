...
export default function Timeline({
  ...
  return (
    <ol className="relative space-y-4 pl-6">
      {/* đường dọc */}
      <span className="absolute left-[7px] top-1 bottom-1 w-0.5 bg-slate-200" aria-hidden />
      {items.map((entry, i) => {
          ...
          <li key={`${entry.timestamp}-${i}`} className="relative">
            <span
              className={`absolute -left-6 top-1 h-4 w-4 rounded-full border-2 ${
                isLatest ? 'border-coral bg-coral' : 'border-slate-300 bg-white'
              }`}
              aria-hidden
            />
            <div className="flex flex-wrap items-center gap-2">
              ...
              {entry.updated_by && names[entry.updated_by] && (
                <span className="text-xs text-slate-500">· {names[entry.updated_by]}</span>
              )}
            </div>
